...
public class View extends JFrame{
	...
	public View() { // 생성자
		...
		JPanel resultDetail = new JPanel(); // 추가적인 정보를 보여주기 위한 패널(미구현)
		...
		btn[0] = new JButton("관객 수"); // 각각의 버튼 생성
		...
		btn[7].setBackground(Color.white); // 마지막 버튼은 초기화 버튼이므로 따로 설정
		btn[7].setFont(new Font("HY바다M", 0, 27));
		...
		DecimalFormat form = new DecimalFormat("#,###"); // 천의 자리마다 쉼표를 찍기 위한 객체
		
		btn[0].addActionListener(new ActionListener() { // 관객 수 기준 분석
			
			public void actionPerformed(ActionEvent arg0) {
				
				current.setText(chooseStandard(monthChoose, nationChoose, ratingChoose) + " 관객 수 분석"); // 콤보 박스로 선택한 기준 적용
				datas = md.sortPNum(monthChoose.getSelectedItem().toString(), // 분석한 결과를 받아서 저장
	...
	public String chooseStandard(JComboBox monthChoose, JComboBox nationChoose, JComboBox ratingChoose) { // 콤보 박스에서 선택한 값들에 대한 조합
		...
		return selectedMonth + selectedNation + selectedRating; // 문자열 조합 후 반환
